...
Exports ActionButton (onClick), StatCard, EmptyState (onAction), and CodeEditor (onChange / onFocus / onBlur). Props mirror the Vue package; className and style apply to the enclosing div. No React children, scoped slots, automatic context inheritance, or server-rendered Vue content. Each adapter owns a Vue app and cleans it up on React unmount. For Next.js, use within a client component.

...
const CodeEditorPlayground = defineAsyncComponent(() => import('./CodeEditorPlayground.vue'))
...
function keyboard(e: KeyboardEvent) { const target = e.target as HTMLElement; if (e.key === '/' && !target.isContentEditable && !['INPUT', 'TEXTAREA', 'SELECT'].includes(target.tagName)) { e.preventDefault(); page.value = 'components'; search.value?.focus() } if (e.key === 'Escape') { mobileNav.value = false; search.value?.blur() } }
function readHash() { const [p, id] = location.hash.slice(1).split('/'); if (p === 'gantt') { choose('gantt'); return }; if (['components','guide','release','changelog'].includes(p)) page.value = p as Page; if (catalog.some(c => c.id === id)) selected.value = id as ComponentId }
...
<template>
  <div class="workspace">
    <aside :class="['sidebar', { open: mobileNav }]">
      ...
      <button :class="['nav-item', { active: page === 'components' }]" @click="navigate('components')"><LayoutGrid :size="17" />组件总览<span class="nav-count">05</span></button>
      ...
      <div class="side-section component-label">组件目录 <span>5</span></div>

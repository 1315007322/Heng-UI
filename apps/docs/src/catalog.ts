...
import ganttSource from '../../../packages/vue/src/gantt/GanttChart.vue?raw'
...
,
  { id: 'gantt', name: 'GanttChart', title: '资源甘特图', category: '数据展示', desc: '拖拽排程，让资源与时间清晰有序。', details: '支持资源排程、拖动与时长调整，提供约束校验和虚拟滚动。通过右侧参数探索真实交互。', source: ganttSource, props: [['rows', 'GanttRow[]', '必填', '资源与任务数据'], ['start / end', 'string', '必填', '时间轴起止范围'], ['height', 'number', '420', '视口高度'], ['zoomPercent', 'number', '100', '时间轴缩放百分比'], ['readonly', 'boolean', 'false', '全图只读'], ['allowOverlap', 'boolean', 'false', '允许任务重叠'], ['unavailableTimeRanges', 'GanttUnavailableRange[]', '[]', '不可用时段'], ['shifts', 'GanttShift[]', '[]', '班次表头']], event: 'task-change / change-rejected', eventDesc: '父级应用变更，或接收约束拒绝原因。' }
] as const
